...
int CHudHealth::Draw(float flTime)
{
	...
	if(CVAR_GET_FLOAT("rez_timer")){
			...
			gHUD.DrawHudString(75, ScreenHeight/2, 256, ".", 255, 0, 0);
			gHUD.DrawHudNumber(95, ScreenHeight/2, HUDELEM_ACTIVE, time_millisecond, 255, 0, 0);
	...
	if(CVAR_GET_FLOAT("rez_speedometer")){
		plr = gEngfuncs.GetLocalPlayer();
		plr_speed = plr->curstate.velocity[0] + plr->curstate.velocity[1];
		gHUD.DrawHudNumber(ScreenWidth/2, ScreenHeight-64, HUD_ACTIVE, plr_speed, 255, 0, 0);
	}
	if(CVAR_GET_FLOAT("rez_origin_x")){
		plr = gEngfuncs.GetLocalPlayer();
		gHUD.DrawHudNumber(0, 0, HUD_ACTIVE, plr->curstate.origin.x, 255, 0, 0);
	}
	if(CVAR_GET_FLOAT("rez_origin_y")){
		plr = gEngfuncs.GetLocalPlayer();
		gHUD.DrawHudNumber(128, 0, HUD_ACTIVE, plr->curstate.origin.y, 255, 0, 0);
	}
	if(CVAR_GET_FLOAT("rez_origin_z")){
		plr = gEngfuncs.GetLocalPlayer();
		gHUD.DrawHudNumber(368, 0, HUD_ACTIVE, plr->curstate.origin.z, 255, 0, 0);
	}
