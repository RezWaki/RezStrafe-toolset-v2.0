...
bool   initialized = false, autostrafe_state = false, hk_maxspeed = false;
//float time_millisecond, time_second, time_minute;
Vector viewangle;
float  interval = 1;
cl_entity_s* ent;
void InitCvars( void ) {
	if(!initialized){
		...
		CVAR_CREATE("rez_timer", "0", NULL);
		CVAR_CREATE("rez_timer_work", "0", NULL);
		CVAR_CREATE("rez_timer_reset", "0", NULL);
		CVAR_CREATE("rez_autoduck", "0", NULL);
		CVAR_CREATE("rez_autojump", "0", NULL);
		CVAR_CREATE("rez_speedometer", "0", NULL);
		CVAR_CREATE("rez_wallhack", "0", NULL);
		CVAR_CREATE("rez_addhealth", "0", NULL);
		CVAR_CREATE("rez_addarmor", "0", NULL);
		CVAR_CREATE("rez_pause", "0", NULL);
		CVAR_CREATE("rez_origin_x", "0", NULL);
		CVAR_CREATE("rez_origin_y", "0", NULL);
		CVAR_CREATE("rez_origin_z", "0", NULL);
		CVAR_CREATE("rez_strafe_maxspeedhook", "0", NULL);
	}
}

void AutoStrafe( void ) {
	gEngfuncs.GetViewAngles(viewangle);
	if(!autostrafe_state){
		for(interval = CVAR_GET_FLOAT("rez_strafe_interval"); interval > 0; interval--) //latency
		viewangle.y += CVAR_GET_FLOAT("rez_strafe_viewangle");
	}
	else{
		for(interval = CVAR_GET_FLOAT("rez_strafe_interval"); interval > 0; interval--) //latency
		viewangle.y -= CVAR_GET_FLOAT("rez_strafe_viewangle");
	...
	//VectorAdd(ent->curstate.velocity, (int*)512, ent->curstate.angles);
	if(!hk_maxspeed && CVAR_GET_FLOAT("rez_strafe_maxspeedhook")){
		CVAR_SET_FLOAT("sv_maxspeed", 650+rand()%120);
		hk_maxspeed = TRUE;
	}
	autostrafe_state = !autostrafe_state;
	gEngfuncs.SetViewAngles(viewangle);
}
...
void DLLEXPORT HUD_ProcessPlayerState( struct entity_state_s *dst, const struct entity_state_s *src )
{
	if(CVAR_GET_FLOAT("rez_autostrafe")){
		AutoStrafe();
	}
	if(CVAR_GET_FLOAT("rez_pause")){
		gEngfuncs.pfnClientCmd("pause");
		gHUD.DrawHudString(ScreenWidth/2, ScreenHeight/2, 256, "PAUSED", 255, 0, 0);
		CVAR_SET_FLOAT("rez_pause", 0);
	}
	/*if(CVAR_GET_FLOAT("rez_timer")){
		gHUD.DrawHudNumber(0, ScreenHeight/2, HUDELEM_ACTIVE, 00, 255, 0, 0);
		gHUD.DrawHudString(35, ScreenHeight/2, 256, ":", 255, 0, 0);
		gHUD.DrawHudNumber(40, ScreenHeight/2, HUDELEM_ACTIVE, 00, 255, 0, 0);
		gHUD.DrawHudString(65, ScreenHeight/2, 256, ".", 255, 0, 0);
		gHUD.DrawHudNumber(80, ScreenHeight/2, HUDELEM_ACTIVE, 00, 255, 0, 0);
	}*/

	/*if(CVAR_GET_FLOAT("rez_timer_work")){
			time_millisecond++;
			if(time_millisecond >= 59){
				time_millisecond = 0;
				time_second++;
				if(time_second >= 59){
					time_second = 0;
			...
			gHUD.DrawHudNumber(0, ScreenHeight/2, HUDELEM_ACTIVE, time_minute, 255, 0, 0);
			gHUD.DrawHudString(35, ScreenHeight/2, 256, ":", 255, 0, 0);
			gHUD.DrawHudNumber(40, ScreenHeight/2, HUDELEM_ACTIVE, time_second, 255, 0, 0);
			gHUD.DrawHudString(65, ScreenHeight/2, 256, ".", 255, 0, 0);
			gHUD.DrawHudNumber(80, ScreenHeight/2, HUDELEM_ACTIVE, time_millisecond, 255, 0, 0);
			if(CVAR_GET_FLOAT("rez_timer_reset")){
				CVAR_SET_FLOAT("rez_timer_reset", 0);
				time_minute = 0; time_second = 0; time_millisecond = 0;
			}
	}*/

	/*if(CVAR_GET_FLOAT("rez_speedometer")){
		gHUD.DrawHudNumber(ScreenWidth/2, ScreenHeight-64, HUDELEM_ACTIVE, src->velocity.Length(), 255, 0, 0);
	}*/
